...
Actualización posterior: ADR-001 aceptó TypeScript y Node.js `24.x`, ADR-002 el monolito modular, ADR-003 PostgreSQL con baseline 18.x, ADR-004 la topología shared-schema y ADR-009 el repositorio único evolutivo con workspaces bajo demanda el 2026-07-21. ADR-005 aceptó con condiciones NestJS `11.x` como shell, Express y REST/HTTP JSON mínima el 2026-07-22, después de la evidencia aprobada de SPIKE-009. Ese mismo día, DEC-004 aceptó Node.js/pnpm/lockfile/ESM/TypeScript/build/Linux como selección con materialización y evidencia pendientes. Wildcard sigue propuesto; Next.js debe dividirse o limitarse a una superficie aprobada; contenedores no obligan Docker Compose/GitHub Actions; y ADR-005/009 por sí solos no aceptan package manager, Turborepo, Nx ni caché.
