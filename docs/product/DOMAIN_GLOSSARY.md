...
| **Título canónico vigente** | Nombre principal mutable que SR Taller presenta para un CatalogItem estable. | **Aceptado para PBI-041.** Cambiarlo no cambia `itemId`; un Candidate sólo puede adoptarlo mediante decisión explícita aplicada atómicamente. |
| **Título observado de proveedor** | Texto exacto observado en un SupplierListing, con Source, Version y provenance propios. | **Aceptado para PBI-041.** Puede localizar el mismo `itemId` después de un mapping publicado, pero nunca se convierte en alias global ni equivalencia de vocabulario. |
...
| **SupplierReconciliationMemory** | Proyección reconstruible de mappings históricos confirmados por Source y firma exacta. | **Aceptado para EPIC-015.** Puede resolver identidad cuando su provenance publicada es exacta, única, consistente, sin correcciones y compatible; nunca publica ni crea alias. |
| **Candidate match de proveedor** | Sugerencia read-only, Source-scoped y acotada que aparece cuando falla la identidad exacta. | **Aceptado para PBI-041.** El score sólo filtra y ordena; siempre exige decisión humana y no aprende hasta un publish exitoso. |
