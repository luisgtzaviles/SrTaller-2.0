...
- Extensión PBI-023: 38 (6 positivos, 32 negativos).
- Vigente tras Paso 6: **136 (18 positivos, 118 negativos)**.
...
| D5-R045 | config consumida y connection exacta puede diferir consumer sólo hasta transaction step | archivo desconocido y otra facility registrada sin consumer |
| D5-R046 | SQL sólo en migration; probe literal owner-scoped; shadowing/local query ignorados | probe cambiado, tagged alias, namespace `.raw` y `PoolClient.query` tipado |
