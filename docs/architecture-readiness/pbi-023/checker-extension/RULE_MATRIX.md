...
| D5-R045 | Registered facility API and consumer | Archivo DB exige owner, exports exactos y consumer materializado o transición exacta | config consumida por connection; demás facilities registradas/consumidas | path/API/runner/placeholder desconocido o sin consumidor | sólo `database-connection.ts` materializado puede diferir consumer hasta transaction step | `mutation:D5-R045:persistence-boundary` |
| D5-R046 | Raw SQL only in authorized migration | Procedencia de `kysely.sql`, `.raw` o todo `query()` sobre executor tipado | migración central con nombre válido | tagged/raw/query fuera de migrations | sólo `runConnectionVerification(client)` en connection owner con literal exacto `select 1` | `mutation:D5-R046:persistence-boundary` |
