...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `tenancy` | Frontera raíz type-only y módulo Nest de composición | Arquitectura | Autoridad de dominio sobre configuración organizacional; persona pendiente de asignación | Ingeniería; persona pendiente | `TenancyModuleContract` — marcador type-only de la frontera | `stations`, `access` | Ninguna | `stations`, `access`, internals ajenos y todo módulo no aprobado |
