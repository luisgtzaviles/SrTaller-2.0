...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `tenancy` | Frontera raíz y configuración organizacional Tenant | Arquitectura | Autoridad de dominio sobre configuración organizacional; persona pendiente de asignación | Ingeniería; persona pendiente | `TenancyModuleContract`, `TenantId`, `parseTenantId`, `OperatingCurrency`, `parseOperatingCurrency`, `TENANT_SETTINGS_RUNTIME`, `TenantSettingsRuntime`, `TENANT_BOOTSTRAP_PERSISTENCE`, `TenantBootstrapPersistence`, `TenantBootstrapWriter`, `TENANT_LIFECYCLE_COMMIT_RUNTIME`, `TenantLifecycleCommitRuntime`, `TenantLifecycleCommitError`, `TenantLifecycleRecord` — identidad nominal, moneda, bootstrap y transición owner-scoped dentro de una transacción opaca | `stations`, `access`, `users`, `customers`, `catalog` | Ninguna | Internals ajenos y todo módulo no aprobado |
| `stations` | Frontera del contexto operativo confiable, inventario/enrollment de dispositivos, Branch IANA y módulo Nest de composición | Arquitectura | Autoridad de dominio sobre contexto operativo, dispositivos y sucursales; persona pendiente de asignación | Ingeniería; persona pendiente | `StationsModuleContract`, `TrustedStationContext`, `TrustedStationContextError`, `TrustedStationContextResolver`, `TRUSTED_STATION_CONTEXT_RESOLVER`, `TrustedStationAdmissionSnapshot`, `TrustedStationAdmissionValidator`, `TRUSTED_STATION_ADMISSION_VALIDATOR`, `ADMIN_INVITATION_BRANCH_COMMIT_VALIDATOR`, `AdminInvitationBranchCommitValidator`, `BRANCH_SETTINGS_RUNTIME`, `BranchSettingsRuntime`, `BranchSettingsScope`, `BRANCH_ADMINISTRATION_RUNTIME`, `BranchAdministrationRuntime`, `STATION_ADMINISTRATION_RUNTIME`, `StationAdministrationContext`, `StationAdministrationRuntime`, `StationInventoryItem`, `StationEnrollmentItem`, `IssuedStationEnrollment`, `BranchTimeZone`, `OperationalDateTime`, `branchLocalCalendarBoundaryToUtc`, `branchLocalCalendarDate`, `branchLocalDateTimeToUtc`, `parseBranchTimeZone`, `presentOperationalDateTime`, `isTrustedStationContext` — resolución, administración Branch V1, inventario/enrollment de Station, calendario IANA y validación owner-scoped sin exponer tablas ni driver | `access`, `repairs` | `tenancy` | `access`, `repairs`, internals de `tenancy` y todo módulo no aprobado |
...
| `users` | Directorio tenant-scoped, lifecycle de identidad y preferencias personales; sin roles, PIN ni sesión | Arquitectura | Identity Foundation; persona pendiente de asignación | Ingeniería; persona pendiente | `UsersModuleContract`, `ADMIN_INVITATION_USER_COMMIT_RUNTIME`, `AdminInvitationUserCommitRuntime`, `AuthenticationUserReader`, `AUTHENTICATION_USER_READER`, `AuthenticationUserRecord`, `AuthenticationUserScope`, `AuthenticationUserAdmissionSnapshot`, `AuthenticationUserAdmissionValidator`, `AUTHENTICATION_USER_ADMISSION_VALIDATOR`, `TENANT_BOOTSTRAP_USER_WRITER`, `TenantBootstrapUserWriter`, `UserProductRuntime`, `USER_PRODUCT_RUNTIME`, `NewRepairFormMode`, `UserPreferencesRuntime`, `USER_PREFERENCES_RUNTIME` — lectura, lifecycle, bootstrap, invitación administrativa y preferencias owner-scoped dentro de composición | `access` para elegibilidad, bootstrap, administración product-local y preferencia self-service autenticada; `AppModule` para composición | `tenancy` por superficie pública permitida | Roles, PIN, sesión, autorización e internals de tenancy |
...
TL-06 añade commits owner-scoped para invitaciones mediante
`ADMIN_INVITATION_BRANCH_COMMIT_VALIDATOR` y
`ADMIN_INVITATION_USER_COMMIT_RUNTIME`; `access` coordina estos contratos con
`TENANT_LIFECYCLE_COMMIT_RUNTIME` sin escribir tablas de `stations`, `users` o
`tenancy` directamente.
...
policy v10, que `StationsModule` importe `TenancyModule` para el commit atómico
de activación, que `RepairsModule` importe exactamente `AccessModule` y
...
`AccessModule`, `RepairsModule` y `RegistrationModule` componen sus controllers registrados. Cada controller pertenece
