...
El 2026-07-22 se ejecutó [SPIKE-009](../../../spikes/spike-009-nestjs-shell/RESULTS.md), autorizado por Arquitectura + Ingeniería como `Mandatory before acceptance` de ADR-005. Tras el dictamen Opción B se ejecutó la remediación obligatoria con `REMEDIATIONS PASS`; su re-revisión enfocada sigue pendiente. `DEC-004` continúa abierta por esa revisión, la decisión de ADR-005, package manager, lockfile y baseline ejecutable final.
...
| 2 — parcial | DEC-004 | Arquitectura + Ingeniería; re-revisión obligatoria de Seguridad + Operaciones + Calidad para SPIKE-009 | ADR-001/003/009 Accepted, alcance R0, restricciones del repositorio y remediación `REMEDIATIONS PASS` | Remediaciones re-revisadas; ADR-005 resuelto; package manager, lockfile y compatibilidad de la [baseline](DEC-004_BASELINE_TECNICA.md) verificados | Existentes | Lenguaje, runtime, motor, repositorio y remediación satisfechos; re-revisión, framework y baseline ejecutable pendientes | H0 |
