...
| ADR-005 | NestJS para backend y API | Proposed | H0 | Ejecutar y revisar SPIKE-009, `Mandatory before acceptance`; después aceptar, condicionar, iterar o rechazar |
...
| 1 | Framework backend y API inicial | DEC-004, ADR-001/002/003/004/009–013 | No; Arquitectura + Ingeniería ya autorizaron el experimento sintético | Sí: [SPIKE-009](../../reviews/sprint-00/PROTOTYPE_CANDIDATES.md#spike-009), obligatorio y todavía no ejecutado | Evidencia revisada y decisión explícita de ADR-005; NestJS/Express/REST permanecen hipótesis hasta entonces | H0 |
...
ADR-005 es la siguiente revisión de plataforma necesaria para avanzar `DEC-004`. Su siguiente paso autorizado es ejecutar [SPIKE-009](../../reviews/sprint-00/PROTOTYPE_CANDIDATES.md#spike-009) bajo el mandato formal, obtener la revisión obligatoria de Seguridad, Operaciones y Calidad y someter la evidencia a Arquitectura + Ingeniería. Debe resolver el framework del backend sin inferir package manager, lockfile, estructura física ni autorización de implementación. ADR-006, ADR-007 y ADR-008 conservan sus gates posteriores y no se aceptan por dependencia de ADR-009.
...
ADR-001, ADR-003 y ADR-009 están `Accepted` y satisfacen lenguaje/runtime, motor y topología de repositorio dentro del lote mínimo de plataforma. PostgreSQL 18.x es la baseline de R0 y PostgreSQL 18.4 la versión efectiva inicial. NestJS `11.1.28`, Express y REST/HTTP JSON mínima son únicamente la baseline hipotética de SPIKE-009. `DEC-004` continúa pendiente de la ejecución/revisión del spike, ADR-005, package manager, lockfile y baseline ejecutable final; el primer cambio de implementación de R0 permanece bloqueado. `DEC-002` y `DEC-062` aportan alcance y aceptación esperada, pero tampoco sustituyen los demás gates técnicos y organizacionales.
