...
function captured(value: string): string | null { return value.startsWith('captured:') ? value.slice('captured:'.length) : null; }
function capturedSelection(value: string): string { return `captured:${value}`; }
    ...
    <article className={styles.priceCard}>
      <div className={styles.cardMain}>
        ...
        <div>
          ...
          <p>{kindLabels[value.item.kind]} · {value.item.category.name}{value.item.category.reconciliationStatus === 'PENDING' ? ' (Por revisar)' : ''}{value.item.brand ? ` · ${value.item.brand.name}${value.item.brand.reconciliationStatus === 'PENDING' ? ' (Por revisar)' : ''}` : ''}</p>
...
export function PriceListPage({ capabilities, administrationCapabilities, csrfToken }: Readonly<{ capabilities: readonly OperationalCapability[]; administrationCapabilities: readonly OperationalCapability[]; csrfToken: string }>): React.JSX.Element {
  ...
  const openManage = async (itemId: string): Promise<void> => {
    ...
    try { const item = await getCatalogItem(itemId); setSelected(item); setKind(item.kind); setTitle(item.title); setDescription(item.description ?? ''); setFormCategoryId(item.category.categoryId ?? capturedSelection(item.category.name)); setFormBrandId(item.brand?.brandId ?? (item.brand ? capturedSelection(item.brand.name) : '')); setFormStatus(item.status); setDialog('manage'); }
  ...
  const submitCreate = async (): Promise<void> => {
    ...
    setSaving(true);
    try {
      const categoryCapturedValue = captured(formCategoryId); const brandCapturedValue = captured(formBrandId);
      const created = await createCatalogItem({ kind, title, description: description || null, categoryId: categoryCapturedValue ? null : formCategoryId, categoryCapturedValue, brandId: brandCapturedValue ? null : formBrandId || null, brandCapturedValue, sku: formSku || null, barcode: formBarcode || null, basePriceAmountMinor: price, referenceCostAmountMinor: cost, referenceCostSourceType: 'MANUAL', expectedVersion: 0, clientRequestId: nextRequestId() }, csrfToken);
      ...
      <Dialog open={dialog !== null} size="wide" title={dialog === 'create' ? 'Nuevo artículo' : `Administrar ${selected?.title ?? 'artículo'}`} description={dialog === 'create' ? 'Identidad comercial Tenant-wide y precio base.' : 'Los cambios de importe crean revisiones; el override sólo afecta esta sucursal.'} onClose={close} footer={false}>
        <div className={styles.dialogBody}>
          ...
          <section className={styles.formGrid} aria-label="Identidad del artículo">
            ...
            <Field id="catalog-category" label="Categoría" required hint="Si no existe, el texto se captura Por revisar; todavía no crea una categoría canónica."><CatalogReferenceCombobox id="catalog-category" label="Categorías aplicables" emptyLabel="Buscar categoría…" value={formCategoryId} references={applicableCategories} canCreate={canManage && dialog === 'create'} onChange={setFormCategoryId} onCapture={captureCategory} /></Field>
            <Field id="catalog-brand" label="Marca" hint="Opcional; un valor nuevo se captura Por revisar."><CatalogReferenceCombobox id="catalog-brand" label="Marcas aplicables" emptyLabel="Sin marca / buscar…" value={formBrandId} references={applicableBrands} canCreate={canManage && dialog === 'create'} onChange={setFormBrandId} onCapture={captureBrand} /></Field>
