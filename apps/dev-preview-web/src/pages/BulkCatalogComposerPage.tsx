...
export function BulkCatalogComposerPage({ capabilities, csrfToken, timeZone }: Readonly<{ capabilities: readonly OperationalCapability[]; csrfToken: string; timeZone: string }>): React.JSX.Element {
  ...
  const comparisonCandidates = current ? versions.filter((value) => value.sourceId === current.sourceId && value.sequenceNumber < current.sequenceNumber && (current.completeness === 'PARTIAL' || value.completeness === 'COMPLETE')) : [];
    ...
    <div className={`${styles.layout} ${!sourcesOpen ? styles.layoutCollapsed : ''}`}>
      ...
      <main className={styles.composer}>
        <section className={styles.setup}>
          ...
          <fieldset disabled={current?.lifecycle === 'INGESTED'}><legend>Alcance de la carga</legend><label><input type="radio" checked={completeness === 'PARTIAL'} onChange={() => { setCompleteness('PARTIAL'); setDirty(true); }} />Actualización parcial<small>Solo estás cargando algunos artículos. Lo que no aparezca no se interpretará como ausencia.</small></label><label><input type="radio" checked={completeness === 'COMPLETE'} onChange={() => { setCompleteness('COMPLETE'); setDirty(true); }} />Lista completa del proveedor<small>Esta carga representa la lista completa recibida del proveedor. Los artículos que falten podrán mostrarse como no observados, pero no se desactivarán.</small></label></fieldset>
        </section>
        ...
        {current ? <section className={styles.comparison}><h2><GitCompare size={18} />Comparar versiones</h2><Select value={compareId} onChange={(event) => setCompareId(event.target.value)}><option value="">Versión anterior…</option>{comparisonCandidates.map((value) => <option key={value.versionId} value={value.versionId}>v{value.sequenceNumber}{value.description ? ` · ${value.description}` : ''}</option>)}</Select><Button size="compact" onClick={() => void compareVersions()} disabled={!compareId}>Comparar</Button>{current.completeness === 'PARTIAL' ? <p>Esta es una actualización parcial; los artículos ausentes no se evalúan.</p> : comparisonCandidates.length === 0 ? <p>No existe una lista completa anterior para comparar ausencias.</p> : null}{compare ? <p>Mapeadas {compare.mapped} · Cambiadas {compare.changed} · Nuevas {compare.added} · {compare.absenceStatus === 'EVALUATED' ? `No observados ${compare.notObserved ?? 0} · ` : ''}Ambiguas {compare.ambiguous}</p> : null}</section> : null}
