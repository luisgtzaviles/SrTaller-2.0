...
export function BulkCatalogComposerPage({ capabilities, csrfToken, timeZone }: Readonly<{ capabilities: readonly OperationalCapability[]; csrfToken: string; timeZone: string }>): React.JSX.Element {
  ...
  const resetCoverageDetails = (): void => { setContinuedOpen(false); setNotObservedOpen(false); setAdditionalOpen(false); setCoverageReviewOpen(false); };
  ...
  const save = async (): Promise<void> => {
    ...
    try {
      ...
      setCurrent(saved); setRows(fromRecord(saved)); resetCoverageDetails(); setDirty(false); setValidationAttempted(false); setServerIssues([]); undoRows.current = null; await refresh(); showToast('Borrador guardado. Puedes continuar después.');
    } catch (error) {
  ...
  const analyze = async (): Promise<void> => { if (!current) return; setBusy(true); try { const value = await analyzeSupplierVersion(current.versionId, current.version, canReadCost, csrfToken); setCurrent(value); setRows(fromRecord(value)); setGridExpanded(false); resetCoverageDetails(); setReconciliationView('ATTENTION'); await refresh(); showToast('Análisis terminado sin escribir en Lista de precios.'); } catch { setNotice({ tone: 'danger', message: 'No fue posible analizar el snapshot.' }); } finally { setBusy(false); } };
  ...
  const coverageItemContext = (item: SupplierVersion['absenceBaseline']['continuedItems'][number]): string => item.observedTitle && item.canonicalTitle && item.observedTitle !== item.canonicalTitle ? `Recibido como “${item.observedTitle}”` : item.status === 'INACTIVE' ? 'Estado actual: Inactivo' : item.status === 'ACTIVE' ? 'Estado actual: Activo' : 'Sin estado actual disponible';
    ...
    <div className={`${styles.layout} ${!sourcesOpen ? styles.layoutCollapsed : ''}`}>
      ...
      <main className={styles.composer}>
        ...
        {current?.lifecycle === 'INGESTED' ? <section className={styles.coverage} aria-labelledby="supplier-coverage-title">
          ...
          {current.absenceBaseline.status === 'NOT_APPLICABLE' ? <><div className={styles.coverageCounts}><div><strong>{current.rowCount.toLocaleString('es-MX')}</strong><span>artículos procesados</span></div></div><p>Los artículos que no fueron incluidos no se evaluaron.</p></> : current.absenceBaseline.status === 'NO_BASELINE' ? <><div className={styles.coverageCounts}><div><strong>{current.rowCount.toLocaleString('es-MX')}</strong><span>filas recibidas</span></div></div><p>No existe una lista completa anterior aplicada para evaluar cobertura.</p></> : <><div className={styles.coverageCounts}><div><strong>{current.absenceBaseline.currentCount.toLocaleString('es-MX')}</strong><span>filas recibidas</span></div><div><strong>{current.absenceBaseline.continuedCount?.toLocaleString('es-MX')}</strong><span>continúan desde la lista anterior</span></div><div><strong>{current.absenceBaseline.notObservedCount?.toLocaleString('es-MX')}</strong><span>ya no aparecen en esta lista completa</span></div><div><strong>{current.absenceBaseline.additionalCount?.toLocaleString('es-MX')}</strong><span>adicionales respecto a la lista anterior</span></div></div><p>Cobertura frente a {current.sourceName} v{current.absenceBaseline.sequenceNumber}, última lista completa aplicada.</p><p>Esta comparación sólo describe lo recibido del proveedor. No desactiva artículos de SR Taller.</p>{current.absenceBaseline.plausibility.status === 'REVIEW_REQUIRED' ? <Alert tone="warning" title="Lista completa mucho más pequeña que la referencia anterior">Referencia anterior: {current.absenceBaseline.plausibility.baselineCount?.toLocaleString('es-MX')} artículos. Lista actual: {current.absenceBaseline.plausibility.currentCount.toLocaleString('es-MX')}. {current.absenceBaseline.plausibility.notObservedCount?.toLocaleString('es-MX')} ya no aparecen. Aplicarla no desactiva artículos, pero la convertirá en la nueva referencia completa del proveedor.</Alert> : null}<div className={styles.coverageActions}>{current.absenceBaseline.continuedItems.length > 0 ? <><Button size="compact" aria-expanded={continuedOpen} aria-controls={continuedPanelId} onClick={() => setContinuedOpen((value) => !value)}>{continuedOpen ? 'Ocultar' : `Ver ${current.absenceBaseline.continuedItems.length.toLocaleString('es-MX')} que continúa${current.absenceBaseline.continuedItems.length === 1 ? '' : 'n'}`}</Button>{continuedOpen ? <ul id={continuedPanelId} className={styles.notObservedItems}>{current.absenceBaseline.continuedItems.map((item, index) => <li key={`${item.itemId ?? coverageItemTitle(item)}-${index}`}><strong>{coverageItemTitle(item)}</strong><span>{coverageItemContext(item)}</span></li>)}</ul> : null}</> : null}{current.absenceBaseline.notObservedItems.length > 0 ? <><Button size="compact" aria-expanded={notObservedOpen} aria-controls={notObservedPanelId} onClick={() => setNotObservedOpen((value) => !value)}>{notObservedOpen ? 'Ocultar' : `Ver ${current.absenceBaseline.notObservedItems.length.toLocaleString('es-MX')} que ya no aparece${current.absenceBaseline.notObservedItems.length === 1 ? '' : 'n'}`}</Button>{notObservedOpen ? <ul id={notObservedPanelId} className={styles.notObservedItems}>{current.absenceBaseline.notObservedItems.map((item, index) => <li key={`${item.itemId ?? coverageItemTitle(item)}-${index}`}><strong>{coverageItemTitle(item)}</strong><span>{coverageItemContext(item)} · Observado en {current.sourceName} v{current.absenceBaseline.sequenceNumber}</span></li>)}</ul> : null}</> : null}{current.absenceBaseline.additionalItems.length > 0 ? <><Button size="compact" aria-expanded={additionalOpen} aria-controls={additionalPanelId} onClick={() => setAdditionalOpen((value) => !value)}>{additionalOpen ? 'Ocultar' : `Ver ${current.absenceBaseline.additionalItems.length.toLocaleString('es-MX')} adicionales`}</Button>{additionalOpen ? <ul id={additionalPanelId} className={styles.notObservedItems}>{current.absenceBaseline.additionalItems.map((item, index) => <li key={`${item.itemId ?? coverageItemTitle(item)}-${index}`}><strong>{coverageItemTitle(item)}</strong><span>{coverageItemContext(item)}</span></li>)}</ul> : null}</> : null}</div></>}
        </section> : null}
