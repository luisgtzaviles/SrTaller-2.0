    ...
    <div className={`${styles.layout} ${!sourcesOpen ? styles.layoutCollapsed : ''}`}>
      ...
      <main className={styles.composer}>
        ...
        <section className={styles.workspaceToolbar}>
          ...
          <div className={styles.toolbarControls}>{mode === 'FULL' ? <div className={styles.presetButtons} role="group" aria-label="Columnas de trabajo"><Button size="compact" tone={viewPreset === 'ESSENTIAL' ? 'primary' : 'quiet'} onClick={() => setViewPreset('ESSENTIAL')}>Esenciales</Button><Button size="compact" tone={viewPreset === 'ALL' ? 'primary' : 'quiet'} onClick={() => setViewPreset('ALL')}>Todas</Button></div> : <span>Actualización compacta</span>}{current?.lifecycle === 'DRAFT' || !current ? <Button size="compact" onClick={() => void save()} disabled={busy}><Save size={17} />Guardar borrador</Button> : null}{current?.lifecycle === 'DRAFT' ? <Button size="compact" tone="primary" onClick={() => void analyze()} disabled={busy}><Check size={17} />Analizar versión</Button> : null}{current?.batch.lifecycle === 'READY' && canPublish ? <Button size="compact" tone="primary" onClick={() => void publish()} disabled={busy}><Send size={17} />Aplicar lote</Button> : null}{current?.batch.lifecycle === 'APPLIED' && canBulkRetire ? <Button size="compact" tone="danger" onClick={() => void planCreatedBatchRetirement()} disabled={busy}><ArchiveX size={17} />Retirar artículos creados por este lote</Button> : null}</div>
