...
export function BulkCatalogComposerPage({ capabilities, csrfToken, timeZone }: Readonly<{ capabilities: readonly OperationalCapability[]; csrfToken: string; timeZone: string }>): React.JSX.Element {
  ...
  const chooseDuplicateRow = async (row: SupplierVersionRow): Promise<void> => {
    ...
    if (await resolve(row.rowDecisionId, row.version, 'APPLY', row.targetItemId)) {
  ...
  return <div className={styles.page}>
    ...
    <Dialog open={supplierGateOpen} title="Nueva carga" description="Elige el proveedor y qué contiene esta carga antes de pegar datos." restoreFocusSelector={`#${supplierGateRestoreTarget}`} onClose={() => { if (!busy) setSupplierGateOpen(false); }} footer={false}>
      <div className={styles.supplierGate}>
        <label>¿De qué proveedor es esta lista?<Input autoFocus type="search" value={supplierSearch} placeholder="Buscar proveedor..." onChange={(event) => setSupplierSearch(event.target.value)} /></label>
        ...
        <fieldset className={styles.loadIntentChoices}><legend>¿Qué contiene esta carga?</legend>{NEW_LOAD_INTENTS.map((intent) => <label key={intent.value} className={supplierGateCompleteness === intent.value ? styles.selectedLoadIntent : ''}><input type="radio" name="new-load-intent" value={intent.value} checked={supplierGateCompleteness === intent.value} onChange={() => setSupplierGateCompleteness(intent.value as SupplierCatalogCompleteness)} /><span><strong>{intent.label}</strong><small>{intent.description}</small></span></label>)}</fieldset>
        <footer><Button onClick={() => setSupplierGateOpen(false)}>Cancelar</Button><Button tone="quiet" onClick={() => openNewSource('GATE')}><Plus size={16} />Crear nuevo proveedor</Button><Button tone="primary" disabled={!canContinueNewLoadGate({ supplierId: supplierGateSupplierId, completeness: supplierGateCompleteness })} onClick={continueNewLoad}>Continuar</Button></footer>
      </div>
    </Dialog>
