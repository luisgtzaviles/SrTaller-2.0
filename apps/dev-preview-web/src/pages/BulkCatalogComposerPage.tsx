...
import { applyBatchDefaults, COLUMN_MAX_WIDTH, COLUMN_MIN_WIDTH, COMPACT_COLUMNS, DEFAULT_COLUMN_WIDTHS, ESSENTIAL_COLUMNS, estimateColumnWidth, fillRows, filterSupplierSources, FULL_COLUMNS, hasMeaningfulComposerWork, nextGridCell, nextValidationIssueIndex, normalizeSupplierTitle, parseClipboardMatrix, parseMoneyToMinor, sortValidationIssues, validateComposerDraft, validationIssueFromApi } from './bulk-catalog-composer-model.mjs';
...
export function BulkCatalogComposerPage({ capabilities, csrfToken, timeZone }: Readonly<{ capabilities: readonly OperationalCapability[]; csrfToken: string; timeZone: string }>): React.JSX.Element {
  ...
  const save = async (): Promise<void> => {
    ...
    try {
      ...
      setCurrent(saved); setPendingNewLoadSupplierId(null); setRows(fromRecord(saved)); resetCoverageDetails(); setDirty(false); setValidationAttempted(false); setServerIssues([]); undoRows.current = null; await refresh(); showToast('Borrador guardado. Puedes continuar después.');
    } catch (error) {
  ...
  return <div className={styles.page}>
    ...
    <div className={`${styles.layout} ${!sourcesOpen ? styles.layoutCollapsed : ''}`}>
      ...
      <div className={styles.sourcePanelControl}><button id="composer-sources-panel-toggle" type="button" className={styles.collapseToggle} aria-expanded={sourcesOpen} aria-controls="composer-sources-panel" aria-label={sourcesOpen ? 'Ocultar fuentes y versiones' : 'Mostrar fuentes y versiones'} title={sourcesOpen ? 'Ocultar fuentes y versiones' : 'Mostrar fuentes y versiones'} onClick={() => setSourcesOpen((value) => !value)}>{sourcesOpen ? <PanelLeftClose size={18} /> : <PanelLeftOpen size={18} />}</button></div>
