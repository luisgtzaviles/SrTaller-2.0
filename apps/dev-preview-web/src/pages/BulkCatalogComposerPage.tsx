...
import { applyBatchDefaults, applyBatchDefaultsToEmptyRows, canContinueNewLoadGate, captureActionState, COLUMN_MAX_WIDTH, COLUMN_MIN_WIDTH, COMPACT_COLUMNS, DEFAULT_COLUMN_WIDTHS, derivePolicyDrivenColumns, estimateColumnWidth, fillRows, filterSupplierSources, FULL_COLUMNS, groupDuplicateResolutionRows, hasMeaningfulComposerWork, NEW_LOAD_INTENTS, nextGridCell, nextValidationIssueIndex, normalizeSupplierTitle, orchestrateReviewList, parseClipboardMatrix, parseMoneyToMinor, sortValidationIssues, validateComposerDraft, validationIssueFromApi } from './bulk-catalog-composer-model.mjs';
...
export function BulkCatalogComposerPage({ capabilities, csrfToken, timeZone }: Readonly<{ capabilities: readonly OperationalCapability[]; csrfToken: string; timeZone: string }>): React.JSX.Element {
  ...
  const activeColumns = mode === 'COMPACT' ? compactColumns : viewPreset === 'ESSENTIAL' ? policyColumns.essential : policyColumns.all;
  const requiredColumns = new Set(policyColumns.required);
  const fullPolicyReady = fieldPolicy !== null;
  ...
  const refresh = useCallback(async () => { const [nextSources, nextVersions, nextReferences] = await Promise.all([listSupplierSources(), listSupplierVersions(), listCatalogReferences()]); setSources(nextSources); setVersions(nextVersions); setReferences(nextReferences); if (!browseSourceId && nextSources[0]) setBrowseSourceId(nextSources[0].sourceId); await refreshFieldPolicy(); }, [browseSourceId, refreshFieldPolicy]);
  ...
  const focusIssue = (issue: ValidationIssue): void => {
    ...
    const targetColumns = mode === 'COMPACT' ? compactColumns : policyColumns.all;
  ...
  const applyDefaultsToEmptyCells = (): void => {
    ...
    setRows((existing) => {
      const result = applyBatchDefaultsToEmptyRows(existing, batchDefaults);
  ...
  return <div className={styles.page}>
    ...
    <div className={`${styles.layout} ${!sourcesOpen ? styles.layoutCollapsed : ''}`}>
      ...
      <main className={styles.composer} aria-busy={busy}>
        ...
        {(mode === 'COMPACT' || fullPolicyReady) ? <section id="bulk-catalog-grid" className={styles.gridRegion} aria-label="Editor tabular del catálogo" hidden={!gridExpanded}>
        ...
        {(mode === 'COMPACT' || fullPolicyReady) && (current?.lifecycle === 'DRAFT' || !current) ? <div className={styles.rowActions}><Button size="compact" disabled={!undoRows.current} onClick={undo}><RotateCcw size={16} />Deshacer</Button><Button size="compact" disabled={rows.length >= 10_000} onClick={() => { setDirty(true); setRows((value) => { rememberUndo(value); return [...value, applyBatchDefaults(blank(), batchDefaults)]; }); }}><Plus size={16} />Agregar fila</Button><Button size="compact" disabled={rows.length === 1} onClick={() => { setDirty(true); setRows((value) => { rememberUndo(value); return value.filter((_, index) => index !== active.row); }); }}><Trash2 size={16} />Quitar fila activa</Button></div> : null}
        ...
        {current?.lifecycle === 'INGESTED' ? <section className={styles.decisions}>
          ...
          {visibleDuplicateResolutionGroups.map((group) => { const comparison = duplicateComparisonFields(group.members, canReadCost); const leader = group.members[0]!; const knownDuplicateIdentity = Boolean(group.members.every((member) => member.targetItemId && member.targetItemId === leader.targetItemId)); const detailsId = `duplicate-details-${leader.rowDecisionId}`; const detailsOpen = Boolean(duplicateDetailsOpen[group.key]); return <section key={group.key} className={`${styles.duplicateResolutionCard} ${group.unresolved ? styles.duplicateResolutionPending : styles.duplicateResolutionResolved}`}>
            {group.unresolved ? <><div className={styles.duplicateResolutionHeading}><div><strong>Artículo repetido con datos diferentes</strong><span>{leader.targetTitle ?? leader.proposal.title ?? leader.supplierObservedTitle ?? 'Artículo reconocido'}</span><small>Encontramos este artículo {group.members.length === 2 ? '2 veces' : `${group.members.length} veces`} en la misma lista. Elige cuál fila quieres usar.</small></div></div>
              ...
              {current.batch.lifecycle !== 'APPLIED' && knownDuplicateIdentity ? <div className={styles.duplicateChoices}>{group.members.map((member) => <Button key={member.rowDecisionId} size="compact" tone="primary" disabled={busy} onClick={() => void chooseDuplicateRow(member)}>Usar fila {member.rowNumber}</Button>)}</div> : null}
            ...
            </> : <><strong>Duplicado resuelto</strong><span>Se usará la fila {group.members.find((member) => member.decision !== 'EXCLUDE')?.rowNumber ?? leader.rowNumber}. Las demás se ignorarán en este lote.</span><Button size="compact" tone="quiet" aria-expanded={detailsOpen} aria-controls={detailsId} onClick={() => setDuplicateDetailsOpen((value) => ({ ...value, [group.key]: !detailsOpen }))}>Ver detalles</Button>{detailsOpen ? <div id={detailsId} className={styles.duplicateDetails}>{group.members.map((member) => <div key={member.rowDecisionId}><strong>Fila {member.rowNumber}</strong><span>{member.decision === 'EXCLUDE' ? 'Ignorada en este lote' : 'Fila elegida para este lote'}</span></div>)}</div> : null}</>}
          </section>; })}
