  ...
  return <div className={styles.page}>
    ...
    <div className={`${styles.layout} ${!sourcesOpen ? styles.layoutCollapsed : ''}`}>
      ...
      <main className={styles.composer}>
        ...
        {current ? <section className={styles.comparison}><h2><GitCompare size={18} />Cobertura de lista completa</h2>{current.absenceBaseline.status === 'NOT_APPLICABLE' ? <p>Actualización parcial: los artículos que no cargues no se evalúan como ausencia.</p> : current.absenceBaseline.status === 'NO_BASELINE' ? <p>Aún no existe una lista completa anterior aplicada para evaluar artículos no observados.</p> : <p>Comparada automáticamente con v{current.absenceBaseline.sequenceNumber}, última lista completa aplicada: {current.absenceBaseline.observed} observados · {current.absenceBaseline.notObserved} no observados. Los artículos no observados permanecen activos.</p>}<h2><GitCompare size={18} />Comparar versiones</h2><Select value={compareId} onChange={(event) => setCompareId(event.target.value)}><option value="">Versión anterior…</option>{comparisonCandidates.map((value) => <option key={value.versionId} value={value.versionId}>v{value.sequenceNumber}{value.description ? ` · ${value.description}` : ''}</option>)}</Select><Button size="compact" onClick={() => void compareVersions()} disabled={!compareId}>Comparar</Button>{compare ? <p>Mapeadas {compare.mapped} · Cambiadas {compare.changed} · Nuevas {compare.added} · {compare.absenceStatus === 'EVALUATED' ? `No observados ${compare.notObserved ?? 0} · ` : 'Ausencias no evaluadas · '}Ambiguas {compare.ambiguous}</p> : null}</section> : null}
