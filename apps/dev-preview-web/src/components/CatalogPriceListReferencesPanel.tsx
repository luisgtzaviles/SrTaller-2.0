...
import { normalizeBrandDisplay } from '../catalog-brand-display.mjs';
...
export function CatalogPriceListReferencesPanel({ csrfToken, capabilities }: Readonly<{ csrfToken: string; capabilities: readonly OperationalCapability[] }>): React.JSX.Element {
  ...
  const canManageCanonical = hasOperationalCapability(capabilities, 'catalog.manage');
  const canGovernPendingBrands = hasOperationalCapability(capabilities, 'catalog.configuration.manage');
  ...
  function actions(reference: CatalogReference) {
    return canManageCanonical ? deriveCatalogLifecycleActions({ idPrefix: `commercial-${id(reference)}`, status: reference.status.toLowerCase() as 'active' | 'inactive', deletable: reference.deletable, busy, onEdit: () => openEditor(reference), onDelete: () => setDeletion(reference), onDeactivate: () => { void transition(reference); }, onReactivate: () => { void transition(reference); } }) : [];
  }
  async function resolve(event: React.FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      setNotice(`“${resolver.rawLabel}” quedó resuelto; el valor observado y su trazabilidad permanecen intactos.`); setResolver(null); await load();
  ...
  return <>
    ...
    <CatalogPanel labelledBy="commercial-reference-title">
      <CatalogHeader id="commercial-reference-title" title={referenceKind === 'category' ? 'Categorías comerciales' : 'Marcas comerciales'} description={referenceKind === 'category' ? 'Cada categoría canónica pertenece a un Tipo; los valores libres se reconcilian después.' : 'Una marca canónica puede aplicar a varios Tipos; los valores observados pendientes se gobiernan por identidad normalizada.'} metadata={<CatalogReconciliationSummary canonicalCount={items.length} pendingCount={pending.length} value={surface} onChange={(value) => { setSurface(value); setSelectedIds([]); }} />} canManage={canManageCanonical || (referenceKind === 'brand' && canGovernPendingBrands)} action={surface === 'canonical' && canManageCanonical ? <>{selectionCompatible ? <Button size="compact" onClick={openMerge}><GitMerge size={16} aria-hidden="true" />Fusionar seleccionados</Button> : null}<Button tone="primary" size="compact" onClick={() => openEditor('new')}><Plus size={16} aria-hidden="true" />Agregar {referenceKind === 'category' ? 'categoría' : 'marca'}</Button></> : undefined} />
      ...
      {loading ? <CatalogLoadingState label={surface === 'canonical' ? 'Cargando referencias canónicas…' : 'Cargando valores por revisar…'} /> : surface === 'canonical' ? <CatalogTable><thead><tr><th><span className={styles.srOnly}>Seleccionar</span></th><th>Referencia</th><th>Aplicable a</th><th>Estado</th><th data-mobile-hidden="true"><CatalogCanonicalUsageHeader description="Artículos vinculados por identidad canónica. No cuenta coincidencias del texto capturado." /></th><th><span className={styles.srOnly}>Acciones</span></th></tr></thead><tbody>{visible.length === 0 ? <CatalogEmptyRow colSpan={6}>No hay referencias {status === 'active' ? 'activas' : status === 'inactive' ? 'inactivas' : 'en el catálogo'}.</CatalogEmptyRow> : visible.map((reference) => { const incompatible = referenceKind === 'category' && selectedCategoryKind !== null && !selectedIds.includes(id(reference)) && reference.applicableKinds[0] !== selectedCategoryKind; return <tr key={id(reference)} data-status={reference.status.toLowerCase()}><td><input type="checkbox" aria-label={`Seleccionar ${reference.name} para fusionar`} checked={selectedIds.includes(id(reference))} disabled={!canManageCanonical || busy || reference.status !== 'ACTIVE' || incompatible} onChange={() => toggleSelected(reference)} /></td><td><CatalogEntityName label={reference.name} secondary={`v${reference.version}`} /></td><td>{reference.applicableKinds.map((kind) => kindLabels[kind]).join(', ')}</td><td><CatalogStatusBadge status={reference.status.toLowerCase() as 'active' | 'inactive'} /></td><td data-mobile-hidden="true"><CatalogUsage count={reference.usageCount ?? 0} singular="artículo" plural="artículos" /></td><td><CatalogRowActions emptyLabel="Sin acciones" actions={actions(reference)} /></td></tr>; })}</tbody></CatalogTable> : <CatalogTable><thead><tr><th>{referenceKind === 'brand' ? 'Marca propuesta' : 'Valor observado'}</th><th>Tipo aplicable</th><th data-mobile-hidden="true">Artículos</th><th data-mobile-hidden="true">Primera / última vez</th><th data-mobile-hidden="true">Proveniencia</th><th><span className={styles.srOnly}>Acciones</span></th></tr></thead><tbody>{pending.length === 0 ? <CatalogEmptyRow colSpan={6}>No hay valores por revisar.</CatalogEmptyRow> : pending.map((item) => { const canResolve = referenceKind === 'brand' ? canGovernPendingBrands : canManageCanonical; const sourceLabels = 'observedInSources' in item ? item.observedInSources : []; const displayName = pendingDisplayName(item); return <tr key={pendingId(item)}><td><CatalogEntityName label={displayName} secondary={referenceKind === 'brand' ? `Observado: ${item.rawLabel} · clave ${item.normalizedKey} · v${item.version}` : `Pendiente · clave ${item.normalizedKey} · v${item.version}`} /></td><td>{pendingKinds(item).map((kind) => kindLabels[kind]).join(', ')}</td><td data-mobile-hidden="true"><CatalogUsage count={item.usageCount} singular="artículo" plural="artículos" /></td><td data-mobile-hidden="true"><small>{date(item.firstSeenAt)}<br />{date(item.lastSeenAt)}</small></td><td data-mobile-hidden="true"><small>{sourceLabels.length > 0 ? `Origen: ${sourceLabels.join(', ')}` : item.capturedBy ?? 'Actor no disponible'}<br />Branch {item.capturedInBranchId.slice(0, 8)}</small></td><td><CatalogRowActions actions={canResolve ? [{ key: 'resolve', label: referenceKind === 'brand' ? `Promover o asignar ${displayName}` : 'Resolver', tone: 'primary', disabled: busy, onClick: () => openResolver(item) }] : []} emptyLabel="Sin permisos" /></td></tr>; })}</tbody></CatalogTable>}
    </CatalogPanel>
    ...
    <Dialog open={resolver !== null} title={referenceKind === 'brand' ? 'Promover marca pendiente' : 'Resolver categoría pendiente'} description={resolver ? `Valor observado: ${resolver.rawLabel} · ${usage(resolver.usageCount)} · alcance Tenant-wide` : ''} onClose={() => !busy && setResolver(null)} footer={false}><form className={styles.editorForm} onSubmit={(event) => { void resolve(event); }}>
