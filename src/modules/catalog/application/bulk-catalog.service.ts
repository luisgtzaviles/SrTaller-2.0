...
import type { BulkCatalogClassification, BulkCatalogDecision, BulkCatalogMode } from '../domain/bulk-catalog.js';
...
export class BulkCatalogService {
  ...
  createDraft(context: CatalogMutationContext, value: unknown) { const body = object(value); const selectedMode = mode(body.mode); const rows = parseBulkRows(body.rows, selectedMode); const rawPayload = raw(body.rawPayload); const signature = requiredText(body.columnSignature, 'columnSignature', 64); if (!/^[a-f0-9]{64}$/u.test(signature)) throw new CatalogInputError('columnSignature'); const sourceId = requiredUuid(body.sourceId, 'sourceId'); const clientRequestId = requiredUuid(body.clientRequestId, 'clientRequestId'); const description = optionalText(body.description, 'description', 500); const requestSha256 = sha256({ sourceId, description, mode: selectedMode, columnSignature: signature, rawPayload, rows, includeReferenceCost: body.includeReferenceCost === true }); return this.repository.createDraft(context, { versionId: newId(), batchId: newId(), sourceId, description, clientRequestId, requestSha256, mode: selectedMode, columnSignature: signature, rawPayload, rows, includeReferenceCost: body.includeReferenceCost === true, occurredAt: new Date() }); }
  replaceDraft(context: CatalogMutationContext, versionId: unknown, value: unknown) { const body = object(value); const selectedMode = mode(body.mode); const rows = parseBulkRows(body.rows, selectedMode); const signature = requiredText(body.columnSignature, 'columnSignature', 64); return this.repository.replaceDraft(context, { versionId: requiredUuid(versionId, 'versionId'), expectedVersion: positiveVersion(body.expectedVersion), description: optionalText(body.description, 'description', 500), columnSignature: signature, rawPayload: raw(body.rawPayload), rows, includeReferenceCost: body.includeReferenceCost === true, occurredAt: new Date() }); }
  ...
  async publish(context: CatalogMutationContext, versionId: unknown, value: unknown, mayWriteCost: boolean) { const body = object(value); const clientRequestId = requiredUuid(body.clientRequestId, 'clientRequestId'); const expectedVersion = positiveVersion(body.expectedVersion); const normalizedVersionId = requiredUuid(versionId, 'versionId'); const currency = await this.readCurrency(context.tenantId); if (!currency || !/^[A-Z]{3}$/u.test(currency)) throw new CatalogConflictError(); const fingerprint = sha256({ versionId: normalizedVersionId, expectedVersion, clientRequestId, currency, mayWriteCost }); return this.repository.publish(context, { versionId: normalizedVersionId, expectedVersion, clientRequestId, requestSha256: fingerprint, currency, mayWriteCost, occurredAt: new Date() }); }
