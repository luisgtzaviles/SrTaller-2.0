...
export class BulkCatalogService {
  ...
  async publish(context: CatalogMutationContext, versionId: unknown, value: unknown, mayWriteCost: boolean) { const body = object(value); const clientRequestId = requiredUuid(body.clientRequestId, 'clientRequestId'); const expectedVersion = positiveVersion(body.expectedVersion); const normalizedVersionId = requiredUuid(versionId, 'versionId'); const acknowledged = coverageReviewAcknowledged(body.coverageReviewAcknowledged); const currency = await this.readCurrency(context.tenantId); if (!currency || !/^[A-Z]{3}$/u.test(currency)) throw new CatalogConflictError(); const fingerprint = sha256({ versionId: normalizedVersionId, expectedVersion, clientRequestId, currency, mayWriteCost, coverageReviewAcknowledged: acknowledged }); return this.repository.publish(context, { versionId: normalizedVersionId, expectedVersion, clientRequestId, requestSha256: fingerprint, currency, mayWriteCost, coverageReviewAcknowledged: acknowledged, occurredAt: new Date() }); }
