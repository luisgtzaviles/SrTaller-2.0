...
type BatchCountsMetadata = Partial<Record<BulkCatalogClassification, number>> & Readonly<{ staleCorrectionVersionId?: unknown }>;
/**
 * Publish remains one serializable transaction. These bounded chunks reduce
 * round-trips for large, atomic batches while keeping every generated VALUES
 * statement well below PostgreSQL's parameter limit.
 */
const publishInsertChunkSizes = Object.freeze({ items: 1_000, identifiers: 2_000, prices: 2_000, costs: 2_000, resolutions: 2_000, memories: 2_000, audit: 1_000 });
const correctionVersionId = (counts: unknown): string | null => {
  const value = (counts as BatchCountsMetadata | null)?.staleCorrectionVersionId;
  return typeof value === 'string' ? value : null;
};
const isStaleByCorrection = (counts: unknown): boolean => correctionVersionId(counts) !== null;
...
async function replaceRows(executor: BulkExecutor, tenantId: string, versionId: string, batchId: string, rows: readonly BulkCatalogRowInput[], now: Date): Promise<void> {
  ...
  for (let offset = 0; offset < rows.length; offset += chunkSize) {
    ...
    await executor.insertInto('catalog_update_row_decisions').values(listings.map((listing, index) => ({ tenant_id: tenantId, row_decision_id: randomUUID(), batch_id: batchId, listing_id: listing.listing_id, row_number: listing.row_number, proposal: slice[index]!, classification: 'INVALID' as const, decision: 'UNRESOLVED' as const, target_item_id: null, expected_item_version: null, preselected_by_memory: false, match_origin: 'NONE' as const, match_algorithm_version: BULK_CATALOG_MATCH_ALGORITHM_VERSION, candidate_matches: JSON.stringify([]), errors: serializeRowErrors(['PENDING_ANALYSIS']), warnings: JSON.stringify([]), lock_version: 1, updated_at: now }))).execute();
  }
}
...
export class KyselyBulkCatalogRepository implements BulkCatalogRepositoryPort {
  ...
  async analyze(context: CatalogMutationContext, input: Readonly<{ versionId: string; expectedVersion: number; includeReferenceCost: boolean; occurredAt: Date }>) {
    return this.transaction(async (db, tx) => {
      ...
      const batch = await db.selectFrom('catalog_update_batches').selectAll().where('tenant_id', '=', context.tenantId).where('version_id', '=', input.versionId).forUpdate().executeTakeFirstOrThrow(); if (batch.lifecycle === 'APPLIED' || isStaleByCorrection(batch.counts)) throw new CatalogConflictError(); const rows = await db.selectFrom('catalog_update_row_decisions').selectAll().where('tenant_id', '=', context.tenantId).where('batch_id', '=', batch.batch_id).orderBy('row_number').execute();
  ...
  async decideMany(context: CatalogMutationContext, input: Readonly<{ versionId: string; expectedBatchVersion: number; classifications: readonly BulkCatalogClassification[]; decision: Exclude<BulkCatalogDecision, 'UNRESOLVED'>; includeReferenceCost: boolean; occurredAt: Date }>) {
    return this.transaction(async (db, tx) => {
      ...
      if (batch.lock_version !== input.expectedBatchVersion || batch.lifecycle === 'APPLIED' || isStaleByCorrection(batch.counts)) throw new CatalogConflictError();
  ...
  async publish(context: CatalogMutationContext, input: Readonly<{ versionId: string; expectedVersion: number; expectedBatchVersion: number; clientRequestId: string; requestSha256: string; currency: string; mayWriteCost: boolean; coverageReviewAcknowledged: boolean; occurredAt: Date }>) {
    return this.transaction(async (db, tx) => {
      ...
      for (let offset = 0; offset < itemWrites.length; offset += publishInsertChunkSizes.items) await db.insertInto('catalog_items').values(itemWrites.slice(offset, offset + publishInsertChunkSizes.items)).execute();
      for (let offset = 0; offset < identifierWrites.length; offset += publishInsertChunkSizes.identifiers) await db.insertInto('catalog_item_identifiers').values(identifierWrites.slice(offset, offset + publishInsertChunkSizes.identifiers)).execute();
      for (let offset = 0; offset < priceWrites.length; offset += publishInsertChunkSizes.prices) await db.insertInto('catalog_base_price_revisions').values(priceWrites.slice(offset, offset + publishInsertChunkSizes.prices)).execute();
      for (let offset = 0; offset < costWrites.length; offset += publishInsertChunkSizes.costs) await db.insertInto('catalog_reference_cost_revisions').values(costWrites.slice(offset, offset + publishInsertChunkSizes.costs)).execute();
      for (let offset = 0; offset < resolutionWrites.length; offset += publishInsertChunkSizes.resolutions) await db.insertInto('catalog_supplier_listing_resolutions').values(resolutionWrites.slice(offset, offset + publishInsertChunkSizes.resolutions)).execute();
      const memoriesToWrite = [...memoryWrites.values()];
      for (let offset = 0; offset < memoriesToWrite.length; offset += publishInsertChunkSizes.memories) await db.insertInto('catalog_supplier_reconciliation_memory').values(memoriesToWrite.slice(offset, offset + publishInsertChunkSizes.memories)).onConflict((conflict) => conflict.columns(['tenant_id', 'source_id', 'identifier_scheme', 'normalized_identifier', 'column_signature']).doUpdateSet((eb) => ({ item_id: eb.ref('excluded.item_id'), last_resolution_id: eb.ref('excluded.last_resolution_id'), last_confirmed_at: eb.ref('excluded.last_confirmed_at'), consistency_state: eb.ref('excluded.consistency_state'), correction_count: eb.ref('excluded.correction_count'), version: eb.ref('excluded.version') }))).execute();
      for (let offset = 0; offset < auditWrites.length; offset += publishInsertChunkSizes.audit) await db.insertInto('catalog_audit_events').values(auditWrites.slice(offset, offset + publishInsertChunkSizes.audit)).execute();
