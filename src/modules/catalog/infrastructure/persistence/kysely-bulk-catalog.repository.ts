...
export class KyselyBulkCatalogRepository implements BulkCatalogRepositoryPort {
  ...
  async analyze(context: CatalogMutationContext, input: Readonly<{ versionId: string; expectedVersion: number; includeReferenceCost: boolean; occurredAt: Date }>) {
    return this.transaction(async (db, tx) => {
      ...
      const memories = await db.selectFrom('catalog_supplier_reconciliation_memory').selectAll().where('tenant_id', '=', context.tenantId).where('source_id', '=', current.source_id).where('column_signature', '=', current.column_signature).execute(); const memoryMap = new Map(memories.map((value) => [`${value.identifier_scheme}:${value.normalized_identifier}`, value]));
      ...
      for (const row of rows) {
        ...
        const matched = [...new Map(identifierKeys.map((key) => idMap.get(key)).filter(Boolean).map((value) => [value!.item_id, value!])).values()]; let target = matched[0] ?? null; let remembered = false;
        ...
        if (historicalRows.some((value) => value.consistency_state === 'CONFLICTED') || candidates.length > 1 || (target && candidates.some((candidate) => candidate.item_id !== target!.item_id))) errors.push('AMBIGUOUS_HISTORY');
          ...
          const item = identifiers.find((value) => value.item_id === candidates[0]!.item_id);
          if (!item) errors.push('HISTORICAL_TARGET_NOT_AVAILABLE');
            ...
            if (target.item_id === item.item_id) {
              remembered = true;
              warnings.push(item.status === 'INACTIVE' ? 'HISTORICAL_INACTIVE_MATCH_REQUIRES_REACTIVATION_CONFIRMATION' : 'HISTORICAL_MATCH_REQUIRES_CONFIRMATION');
            }
          }
        }
        if (target?.status === 'INACTIVE' && !remembered) errors.push('HISTORICAL_ITEM_RETIRED_REQUIRES_REACTIVATION');
        ...
        if (errors.length) classification = errors.includes('AMBIGUOUS_HISTORY') ? 'AMBIGUOUS' : 'CONFLICT';
          ...
          const categoryKnown = categories.some((value) => value.kind === p.kind && value.normalized_name === normalizeReference(p.category) && value.status === 'ACTIVE'); const brandKnown = !p.brand || brands.some((value) => value.normalized_name === normalizeReference(p.brand) && value.status === 'ACTIVE'); classification = categoryKnown && brandKnown ? 'NEW' : 'PENDING_REFERENCE'; decision = classification === 'NEW' ? 'APPLY' : 'UNRESOLVED';
        } else {
          ...
          const proposedCategory = categoryKey ? categories.find((value) => value.kind === (p.kind ?? target!.kind) && value.normalized_name === categoryKey && value.status === 'ACTIVE') : null;
          const proposedPendingCategory = categoryKey ? pendingCategories.find((value) => value.kind === (p.kind ?? target!.kind) && value.normalized_key === categoryKey) : null;
          ...
          else { const changed = (p.description !== null && p.description !== target.description) || (Boolean(proposedCategory) && (proposedCategory!.category_id !== target.category_id || target.pending_category_value_id !== null)) || (Boolean(proposedPendingCategory) && proposedPendingCategory!.pending_category_value_id !== target.pending_category_value_id) || (Boolean(proposedBrand) && (proposedBrand!.brand_id !== target.brand_id || target.pending_brand_value_id !== null)) || (Boolean(proposedPendingBrand) && proposedPendingBrand!.pending_brand_value_id !== target.pending_brand_value_id) || (p.basePriceMinor !== null && p.basePriceMinor !== currentPrices.get(target.item_id)) || (p.referenceCostMinor !== null && p.referenceCostMinor !== currentCosts.get(target.item_id)); classification = target.status === 'INACTIVE' ? 'REACTIVATE' : changed ? 'UPDATE' : 'UNCHANGED'; decision = remembered ? 'UNRESOLVED' : 'APPLY'; }
        }
        counts[classification] += 1; if (decision === 'UNRESOLVED') unresolvedCount += 1;
        analyzedWrites.push({ tenant_id: row.tenant_id, row_decision_id: row.row_decision_id, batch_id: row.batch_id, listing_id: row.listing_id, row_number: row.row_number, proposal: row.proposal, classification, decision, target_item_id: target?.item_id ?? null, expected_item_version: target?.version ?? null, preselected_by_memory: remembered, errors: JSON.stringify(errors), warnings: JSON.stringify(warnings), lock_version: row.lock_version + 1, updated_at: input.occurredAt });
      }
      for (let offset = 0; offset < analyzedWrites.length; offset += 500) await db.insertInto('catalog_update_row_decisions').values(analyzedWrites.slice(offset, offset + 500)).onConflict((conflict) => conflict.columns(['tenant_id', 'row_decision_id']).doUpdateSet((eb) => ({ classification: eb.ref('excluded.classification'), decision: eb.ref('excluded.decision'), target_item_id: eb.ref('excluded.target_item_id'), expected_item_version: eb.ref('excluded.expected_item_version'), preselected_by_memory: eb.ref('excluded.preselected_by_memory'), errors: eb.ref('excluded.errors'), warnings: eb.ref('excluded.warnings'), lock_version: eb.ref('excluded.lock_version'), updated_at: input.occurredAt }))).execute();
  ...
  async decide(context: CatalogMutationContext, input: Readonly<{ versionId: string; rowDecisionId: string; expectedRowVersion: number; decision: BulkCatalogDecision; targetItemId: string | null; includeReferenceCost: boolean; occurredAt: Date }>) {
    return this.transaction(async (db, tx) => { const version = await db.selectFrom('catalog_supplier_catalog_versions').selectAll().where('tenant_id', '=', context.tenantId).where('version_id', '=', input.versionId).executeTakeFirst(); if (!version) throw new CatalogNotFoundError(); if (version.lifecycle !== 'INGESTED') throw new CatalogConflictError(); const batch = await db.selectFrom('catalog_update_batches').selectAll().where('tenant_id', '=', context.tenantId).where('version_id', '=', input.versionId).forUpdate().executeTakeFirstOrThrow(); const row = await db.selectFrom('catalog_update_row_decisions').selectAll().where('tenant_id', '=', context.tenantId).where('row_decision_id', '=', input.rowDecisionId).where('batch_id', '=', batch.batch_id).forUpdate().executeTakeFirst(); if (!row) throw new CatalogNotFoundError(); if (row.lock_version !== input.expectedRowVersion) throw new CatalogConflictError(); let classification = row.classification; let expected = row.expected_item_version; if (input.targetItemId) { if (input.decision !== 'APPLY') throw new CatalogConflictError(); const competing = await db.selectFrom('catalog_update_row_decisions').select('row_decision_id').where('tenant_id', '=', context.tenantId).where('batch_id', '=', batch.batch_id).where('row_decision_id', '!=', input.rowDecisionId).where('target_item_id', '=', input.targetItemId).where('decision', '!=', 'EXCLUDE').executeTakeFirst(); if (competing) throw new CatalogConflictError(); const item = await db.selectFrom('catalog_items').select(['version', 'kind']).where('tenant_id', '=', context.tenantId).where('item_id', '=', input.targetItemId).where('status', '=', 'ACTIVE').executeTakeFirst(); if (!item) throw new CatalogConflictError(); const proposal = row.proposal as BulkCatalogRowInput; if (proposal.kind && item.kind !== proposal.kind) throw new CatalogConflictError(); classification = 'UPDATE'; expected = item.version; } if (input.decision === 'APPLY' && ['AMBIGUOUS', 'CONFLICT', 'INVALID'].includes(classification) && !input.targetItemId) throw new CatalogConflictError(); await db.updateTable('catalog_update_row_decisions').set({ classification, decision: input.decision, target_item_id: input.targetItemId ?? row.target_item_id, expected_item_version: expected, preselected_by_memory: false, errors: input.targetItemId ? [] : row.errors, lock_version: row.lock_version + 1, updated_at: input.occurredAt }).where('tenant_id', '=', context.tenantId).where('row_decision_id', '=', input.rowDecisionId).execute(); const unresolved = await db.selectFrom('catalog_update_row_decisions').select(({ fn }) => fn.countAll<string>().as('count')).where('tenant_id', '=', context.tenantId).where('batch_id', '=', batch.batch_id).where('decision', '=', 'UNRESOLVED').executeTakeFirstOrThrow(); await db.updateTable('catalog_update_batches').set({ lifecycle: Number(unresolved.count) === 0 ? 'READY' : 'RECONCILING', lock_version: batch.lock_version + 1, updated_at: input.occurredAt }).where('tenant_id', '=', context.tenantId).where('batch_id', '=', batch.batch_id).execute(); if (!await guardsCurrent(context, tx)) throw new CatalogAuthorizationChangedError(); return (await readVersion(db, context.tenantId, input.versionId, input.includeReferenceCost))!; });
  }
  async decideMany(context: CatalogMutationContext, input: Readonly<{ versionId: string; expectedBatchVersion: number; classifications: readonly BulkCatalogClassification[]; decision: Exclude<BulkCatalogDecision, 'UNRESOLVED'>; includeReferenceCost: boolean; occurredAt: Date }>) {
    return this.transaction(async (db, tx) => {
      ...
      if (input.decision === 'APPLY' && input.classifications.some((value) => ['AMBIGUOUS', 'CONFLICT', 'INVALID'].includes(value))) throw new CatalogConflictError();
  ...
  async publish(context: CatalogMutationContext, input: Readonly<{ versionId: string; expectedVersion: number; clientRequestId: string; requestSha256: string; currency: string; mayWriteCost: boolean; occurredAt: Date }>) {
    return this.transaction(async (db, tx) => {
      ...
      for (const row of rows) {
        ...
        auditWrites.push({ tenant_id: context.tenantId, audit_id: randomUUID(), branch_id: null, station_id: context.stationId, session_id: context.sessionId, actor_user_id: context.actorUserId, actor_display_name: context.actorDisplayName, capability: 'catalog.import.publish', action: 'catalog.bulk.publish.row', resource_id: itemId, old_version: row.expected_item_version, new_version: itemVersion, change_summary: { batchId: batch.batch_id, rowNumber: row.row_number, classification: row.classification, ...(row.classification === 'REACTIVATE' ? { lifecycle: { before: 'INACTIVE', after: 'ACTIVE' } } : {}) }, result: 'SUCCEEDED', correlation_id: randomUUID(), client_request_id: input.clientRequestId, occurred_at: input.occurredAt });
      }
  ...
  compare(scope: CatalogScope, leftVersionId: string, rightVersionId: string): Promise<SupplierVersionComparison> { return this.execute(async (db) => { const rows = async (id: string) => db.selectFrom('catalog_supplier_listings').select(['normalized_supplier_item_code', 'normalized_signature', 'source_observation']).where('tenant_id', '=', scope.tenantId).where('version_id', '=', id).execute(); const [left, right] = await Promise.all([rows(leftVersionId), rows(rightVersionId)]); const key = (r: { normalized_supplier_item_code: string | null; normalized_signature: string }) => r.normalized_supplier_item_code ? `C:${r.normalized_supplier_item_code}` : `S:${r.normalized_signature}`; const lm = new Map(left.map((r) => [key(r), r])); const rm = new Map(right.map((r) => [key(r), r])); let mapped = 0, changed = 0, added = 0, disappeared = 0, ambiguous = 0; const count = (values: readonly { normalized_supplier_item_code: string | null; normalized_signature: string }[]) => { const m = new Map<string, number>(); for (const r of values) m.set(key(r), (m.get(key(r)) ?? 0) + 1); return m; }; const lc = count(left), rc = count(right); for (const [k, r] of rm) { if ((lc.get(k) ?? 0) > 1 || (rc.get(k) ?? 0) > 1) { ambiguous++; continue; } const l = lm.get(k); if (!l) added++; else { mapped++; if (sha256(l.source_observation) !== sha256(r.source_observation)) changed++; } } for (const k of lm.keys()) if (!rm.has(k)) disappeared++; return Object.freeze({ leftVersionId, rightVersionId, mapped, changed, added, disappeared, ambiguous }); }); }
