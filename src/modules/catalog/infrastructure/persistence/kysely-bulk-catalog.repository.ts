...
import type { BulkCatalogCandidateMatch, BulkCatalogClassification, BulkCatalogDecision, BulkCatalogMatchOrigin, BulkCatalogRowInput } from '../../domain/bulk-catalog.js';
...
async function readVersion(executor: BulkExecutor, tenantId: string, versionId: string, includeReferenceCost: boolean): Promise<SupplierVersionRecord | null> {
  ...
  const rows = decisionRows.map((value) => {
    ...
    return Object.freeze({ rowDecisionId: value.row_decision_id, rowNumber: value.row_number, supplierObservedTitle: value.supplier_title, proposal: includeReferenceCost ? proposal : Object.freeze({ ...proposal, referenceCostMinor: null }), classification: value.classification, decision: value.decision, targetItemId: value.target_item_id, targetTitle: value.target_title, expectedItemVersion: value.expected_item_version, before, preselectedByMemory: value.preselected_by_memory, matchOrigin: value.match_origin, matchAlgorithmVersion: value.match_algorithm_version, candidates: Object.freeze(value.candidate_matches as BulkCatalogCandidateMatch[]), errors: Object.freeze(value.errors as string[]), warnings: Object.freeze(value.warnings as string[]), version: value.lock_version });
  });
...
export class KyselyBulkCatalogRepository implements BulkCatalogRepositoryPort {
  ...
  async analyze(context: CatalogMutationContext, input: Readonly<{ versionId: string; expectedVersion: number; includeReferenceCost: boolean; occurredAt: Date }>) {
    return this.transaction(async (db, tx) => {
      ...
      for (const row of rows) {
        ...
        analyzedWrites.push({ tenant_id: row.tenant_id, row_decision_id: row.row_decision_id, batch_id: row.batch_id, listing_id: row.listing_id, row_number: row.row_number, proposal: row.proposal, classification, decision, target_item_id: target?.item_id ?? null, expected_item_version: target?.version ?? null, preselected_by_memory: trusted, match_origin: matchOrigin, match_algorithm_version: BULK_CATALOG_MATCH_ALGORITHM_VERSION, candidate_matches: JSON.stringify(candidateMatches), errors: JSON.stringify(errors), warnings: JSON.stringify(warnings), lock_version: row.lock_version + 1, updated_at: input.occurredAt });
      }
  ...
  async decide(context: CatalogMutationContext, input: Readonly<{ versionId: string; rowDecisionId: string; expectedRowVersion: number; decision: BulkCatalogDecision; targetItemId: string | null; includeReferenceCost: boolean; occurredAt: Date }>) {
    return this.transaction(async (db, tx) => {
      ...
      let classification = row.classification; let expected = row.expected_item_version; let targetItemId = input.targetItemId ?? row.target_item_id;
