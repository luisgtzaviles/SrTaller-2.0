...
export class KyselyAdminInvitationRepository implements AdminInvitationRepositoryPort {
  ...
  async issue(input: Parameters<AdminInvitationRepositoryPort['issue']>[0], guard: Parameters<AdminInvitationRepositoryPort['issue']>[1]): Promise<AdminInvitationRecord> {
    return this.connection[databaseTransactionCapability]({ isolationLevel: 'serializable', accessMode: 'read write' }, async (raw) => {
      ...
      await this.validateAuthorityAndGrants(db, input.tenantId, input.inviterUserId, input.inviterAdminIdentityId, input.authorityRevision, input.grants);
      await db.insertInto('access_admin_invitations').values({ tenant_id: input.tenantId, invitation_id: input.invitationId, normalized_email: input.normalizedEmail, email_display: input.emailDisplay, target_user_id: input.targetUserId, proposed_display_name: input.proposedDisplayName, inviter_user_id: input.inviterUserId, inviter_admin_identity_id: input.inviterAdminIdentityId, status: 'PENDING', version: 0, authority_revision: input.authorityRevision, intended_grants_digest: digestAdminInvitationGrants(input.grants), expires_at: new Date(at.getTime() + 86_400_000), accepted_at: null, revoked_at: null, created_at: at, updated_at: at }).execute();
  ...
  private async validateAuthorityAndGrants(db: AccessDatabase, tenantId: string, inviterUserId: string, identityId: string, authorityRevision: number, grants: readonly AdminInvitationGrant[]): Promise<void> {
    ...
    if (!issuer || issuer.user_status !== 'active' || issuer.identity_status !== 'active' || issuer.credential_status !== 'active' || issuer.verified_at === null || issuer.admission_revision !== authorityRevision) throw new AdminInvitationError('ADMIN_INVITATION_AUTHORITY_CHANGED');
