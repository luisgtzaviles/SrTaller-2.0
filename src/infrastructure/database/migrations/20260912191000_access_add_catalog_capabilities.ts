...
export async function up(database: Kysely<DatabaseSchema>): Promise<void> {
  await replaceConstraint(database, [...priorCapabilities, ...catalogCapabilities]);
  const createdAt = new Date('2026-09-11T18:10:00.000Z');
  await database.insertInto('access_capabilities').values(
    catalogCapabilities.map((capability_code) => ({ capability_code, created_at: createdAt })),
...
export async function down(database: Kysely<DatabaseSchema>): Promise<void> {
  await database.deleteFrom('access_role_capabilities')
    .where('capability_code', 'in', catalogCapabilities)
    .execute();
