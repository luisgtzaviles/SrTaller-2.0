...
type MigrationOwner = 'database' | 'repairs' | 'stations' | 'tenancy' | 'users';
...
const migrationPattern =
  /^(?<timestamp>\d{14})_(?<owner>database|repairs|stations|tenancy|users)_(?<action>[a-z][a-z0-9]*(?:_[a-z0-9]+)+)\.(?<extension>ts|js)$/u;
