...
type MigrationOwner = 'database' | 'preview' | 'stations' | 'tenancy';
...
const migrationPattern =
  /^(?<timestamp>\d{14})_(?<owner>database|preview|stations|tenancy)_(?<action>[a-z][a-z0-9]*(?:_[a-z0-9]+)+)\.(?<extension>ts|js)$/u;
