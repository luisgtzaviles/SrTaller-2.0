...
test('registration UI exposes required fields, separate legal evidence and safe states', () => {
  ...
  for (const state of ['Revisa tu correo', 'Confirma tu correo', 'Verificando correo', 'El enlace no está disponible', 'Correo verificado']) assert.match(page, new RegExp(state, 'u'));
  ...
  assert.match(page, /async function verify\(\)/u);
  assert.match(page, />Confirmar correo</u);
...
test('public UI preserves keyboard semantics and responsive light/dark layouts', () => {
  ...
  assert.match(styles, /var\(--color-canvas\)/u);
  assert.match(styles, /var\(--color-surface\)/u);
