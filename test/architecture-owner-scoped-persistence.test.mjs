...
test('persistence capability is internal and has only exact adapter consumers', async () => {
  ...
  assert.match(
    source,
    /Pick<DatabaseSchema, 'tenants' \| 'tenant_bootstrap_commands' \| 'tenant_bootstrap_guards' \| 'tenant_lifecycle_events'>/u,
  );
  assert.match(
    source,
  ...
  assert.match(
    source,
    /Pick<DatabaseSchema, 'users' \| 'user_preferences' \| 'user_provisioning_bootstraps' \| 'user_lifecycle_commands' \| 'user_profile_update_commands' \| 'user_create_commands'>/u,
  );
  assert.match(
    source,
