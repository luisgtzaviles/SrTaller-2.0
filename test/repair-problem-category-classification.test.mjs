...
test('reported problems extend the category foundation across intake, reconciliation and later classification', async () => {
  ...
  assert.match(catalog, /CatalogReconciliationSummary/u); assert.match(administration, /por revisar/u); assert.match(administration, /canónicas/u); assert.match(catalog, /deletable/u); assert.match(catalog, /deriveCatalogLifecycleActions/u); assert.match(catalog, /CatalogSafeDeleteDialog/u); assert.match(administration, /Eliminar definitivamente/u); assert.match(administration, /nunca ha sido utilizado/u); assert.doesNotMatch(catalog, /role="tablist" aria-label="Vista de categorías"/u); assert.match(administration, /Plataforma/u); assert.match(administration, /Organización/u);
