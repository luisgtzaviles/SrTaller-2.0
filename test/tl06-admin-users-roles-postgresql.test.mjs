...
test('TL-06 PostgreSQL executes invitation replay, supersession, acceptance and secret-safe audit', { skip: !enabled, timeout: 60_000 }, async () => {
  ...
  try {
    ...
    const challenges = await database.query('select status, expires_at from access_admin_invitation_challenges where tenant_id=$1 order by created_at', [seeded.tenantId]);
    assert.deepEqual(challenges.rows.map(({ status }) => status), ['SUPERSEDED', 'ACTIVE']);
    assert.equal(challenges.rows[1].expires_at.toISOString(), issued.expiresAt);
...
test('TL-06 PostgreSQL fails closed across tenants, branches and stale issuer authority', { skip: !enabled, timeout: 60_000 }, async () => {
  const [{ createDatabaseConnection }, { LocalEmailDelivery }, { AdminInvitationService }, , , { KyselyAdminInvitationRepository }] = modules; const database = pool(); const connection = createDatabaseConnection(config('isolation')); const alpha = await seedTenant(database); const beta = await seedTenant(database); let tokenValue = token('e'); let now = new Date('2026-09-21T22:00:00.000Z');
  try {
    await connection.verify(); const repository = invitationRepository(connection, KyselyAdminInvitationRepository); const service = new AdminInvitationService(repository, passwordHasher(), new LocalEmailDelivery(), 'http://127.0.0.1:4173', () => now, randomUUID, () => tokenValue); const guard = { async confirmCurrent() { return true; } };
    ...
    tokenValue = token('i'); const expiring = await service.issue({ tenantId: alpha.tenantId, inviterUserId: alpha.users[0].userId, inviterAdminIdentityId: alpha.users[0].identityId, targetUserId: null, proposedDisplayName: 'Expiring invite', email: 'expiring@example.test', grants: [{ roleId: alpha.roleId, roleVersion: 0, assignmentScope: 'TENANT_WIDE', branchId: null }], clientRequestId: randomUUID(), correlationId: randomUUID(), guard });
    now = new Date('2026-09-22T22:00:00.001Z');
    assert.equal((await service.list(alpha.tenantId)).find(({ invitationId }) => invitationId === expiring.invitationId)?.status, 'EXPIRED');
    assert.equal((await database.query('select status from access_admin_invitation_challenges where tenant_id=$1 and invitation_id=$2', [alpha.tenantId, expiring.invitationId])).rows[0].status, 'EXPIRED');
    tokenValue = token('j'); const reinvited = await service.issue({ tenantId: beta.tenantId, inviterUserId: beta.users[0].userId, inviterAdminIdentityId: beta.users[0].identityId, targetUserId: null, proposedDisplayName: 'Reinvited elsewhere', email: 'expiring@example.test', grants: [{ roleId: beta.roleId, roleVersion: 0, assignmentScope: 'TENANT_WIDE', branchId: null }], clientRequestId: randomUUID(), correlationId: randomUUID(), guard });
    ...
    assert.equal((await repository.list(beta.tenantId, now.toISOString())).some(({ invitationId }) => invitationId === issued.invitationId), false);
  } finally { await connection.close(); await database.end(); }
});
