...
test('TL-06 PostgreSQL executes invitation replay, supersession, acceptance and secret-safe audit', { skip: !enabled, timeout: 60_000 }, async () => {
  ...
  try {
    ...
    now = new Date('2026-09-21T21:05:00.000Z'); const resent = await service.resend({ tenantId: seeded.tenantId, invitationId: issued.invitationId, expectedVersion: 0, clientRequestId: randomUUID(), correlationId: randomUUID(), guard: input.guard }); assert.equal(resent.version, 1);
    ...
    const material = JSON.stringify((await database.query('select * from access_admin_lifecycle_events where tenant_id=$1', [seeded.tenantId])).rows); assert.equal(material.includes('Synthetic password'), false); assert.equal(material.includes(tokens[2].token), false);
