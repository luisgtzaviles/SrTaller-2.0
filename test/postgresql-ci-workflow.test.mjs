...
test('authoritative workflow runs PostgreSQL in both independent VC-024 jobs', () => {
  ...
  assert.match(
    workflow,
    /name: Run TL-06 PostgreSQL 18\.4 contracts[\s\S]*--stage tl06-postgresql[\s\S]*node scripts\/test-tl06-postgresql\.mjs/u,
  );
  assert.match(
    workflow,
    /name: Run TL-07 PostgreSQL 18\.4 contracts[\s\S]*--stage tl07-postgresql[\s\S]*node scripts\/test-tl07-postgresql\.mjs/u,
  );
  assert.match(
    workflow,
