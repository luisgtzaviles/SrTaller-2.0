...
const execute = promisify(execFile); const suffix = randomBytes(6).toString('hex'); const container = `srtaller_tl04_${suffix}`; const database = `srtaller_tl04_${suffix}`; const user = 'srtaller_tl04_test'; const password = `synthetic_${randomBytes(18).toString('hex')}`; let started = false; let output = '';
...
try {
  ...
  const summary = assertPostgresqlTestSummary(testResult.stdout); assert.equal(summary.pass, 3); output = `${testResult.stdout}TL-04 PostgreSQL PASS: 89 migrations, second run 0 pending\n`;
