...
try {
  ...
  const first = JSON.parse((await execute(process.execPath, ['--enable-source-maps', 'dist/db-migrate.js'], { encoding: 'utf8', env, timeout: 90_000 })).stdout.trim()); assert.equal(first.applied, 80); assert.equal(first.pending, 0);
  ...
  const summary = assertPostgresqlTestSummary(testResult.stdout); assert.equal(summary.pass, 2); output = `${testResult.stdout}TL-04 PostgreSQL PASS: 80 migrations, second run 0 pending\n`;
