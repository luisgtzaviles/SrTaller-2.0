...
if (process.argv.includes('--dry-run')) {
  ...
  const operations = {
    ...
    tl02Postgresql: async () => {
      ...
      if (!result.stdout.includes('TL-02 PostgreSQL PASS: 81 migrations, second run 0 pending')) throw new Error('TL-02 PostgreSQL evidence is incomplete');
    ...
    tl03Postgresql: async () => {
      ...
      if (!result.stdout.includes('TL-03 PostgreSQL PASS: 81 migrations, second run 0 pending')) throw new Error('TL-03 PostgreSQL evidence is incomplete');
    ...
    tl04Postgresql: async () => {
      ...
      if (!result.stdout.includes('TL-04 PostgreSQL PASS: 81 migrations, second run 0 pending')) throw new Error('TL-04 PostgreSQL evidence is incomplete');
