...
if (process.version !== 'v24.18.0') {
  throw new Error('Node.js 24.18.0 is required for schema verification');
}
if (
  ...
  throw new Error(
    'usage: node scripts/test-database-schema-postgresql.mjs [--runs 1..5]',
  );
}
...
async function docker(argumentsList, options = {}) {
  try {
    ...
    throw new Error(
      `Docker schema test operation failed: ${argumentsList[0] ?? 'unknown'}`,
    );
  }
}
...
async function runOnce() {
  ...
  const container = `srtaller_pbi023_schema_${suffix}`;
  ...
  try {
    ...
    await execute(
      ...
      ['--test', 'test/database-schema-postgresql.test.mjs'],
      {
        ...
        env: {
          ...process.env,
          SR_SCHEMA_PG_HOST: '127.0.0.1',
          SR_SCHEMA_PG_NAME: database,
          SR_SCHEMA_PG_PASSWORD: password,
    ...
    if (
      ...
      /tenants|branches|kysely_migration|schema_branch_reference_probe/iu.test(
        schemaOutput,
      )
    ) {
      throw new Error(
        'schema verification unexpectedly retained database objects',
      );
    }
    ...
    return Object.freeze({
      ...
      scenarios: Object.freeze([
        'atomicity',
        'cleanup',
        'composite-future-reference',
        'constraint-errors',
        'dist-execution',
        'down',
        'down-dependency-block',
        'indexes',
        'introspection',
        'journal',
...
if (results.some((result) => JSON.stringify(result) !== material)) {
  throw new Error('PostgreSQL schema verification runs differ materially');
}
