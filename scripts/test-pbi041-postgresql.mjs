...
try {
  ...
  const migrationResult = JSON.parse(migration.stdout.trim()); if (migrationResult.pending !== 0 || migrationResult.applied !== 76) throw new Error('PBI-041 migration set is incomplete');
  ...
  const result = await timing.measure('test-process', () => execute(process.execPath, ['--no-maglev', '--test', '--test-concurrency=1', 'test/bulk-catalog-postgresql.test.mjs', 'test/catalog-authorization-postgresql.test.mjs', 'test/catalog-postgresql.test.mjs'], { encoding: 'utf8', env: { ...process.env, SR_PBI041_PG_TEST: '1', SR_PBI041_AUTH_PG_TEST: '1', SR_PBI041_CATALOG_PG_TEST: '1', SR_PBI041_PERF_DIAGNOSTICS: process.env.SR_PBI041_PERF_DIAGNOSTICS ?? '0', SR_PBI041_PG_HOST: '127.0.0.1', SR_PBI041_PG_PORT: port, SR_PBI041_PG_NAME: database, SR_PBI041_PG_USER: user, SR_PBI041_PG_PASSWORD: password, SR_PBI040_PG_HOST: '127.0.0.1', SR_PBI040_PG_PORT: port, SR_PBI040_PG_NAME: database, SR_PBI040_PG_USER: user, SR_PBI040_PG_PASSWORD: password }, maxBuffer: 20 * 1024 * 1024, timeout: 90_000 }));
