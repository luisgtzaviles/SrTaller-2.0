...
if (!Number.isInteger(requestedRuns) || requestedRuns < 1 || requestedRuns > 5) {
  throw new Error(
    'usage: node scripts/test-database-transaction-postgresql.mjs [--runs 1..5]',
  );
}
...
async function runOnce() {
  ...
  const runId = `transaction_${suffix}`;
  const container = `srtaller_pbi023_transaction_${suffix}`;
  const database = `srtaller_test_${runId}`;
  const user = 'srtaller_transaction_test';
  ...
  try {
    ...
    const testEnvironment = {
      ...
      SR_TEST_DB_APPLICATION_NAME: 'srtaller-pbi023-transaction-postgresql',
      ...
      SR_TRANSACTION_PG_TEST: '1',
    };
    await execute(
      process.execPath,
      ['--test', 'test/database-transaction-postgresql.test.mjs'],
      {
        encoding: 'utf8',
        env: testEnvironment,
        maxBuffer: 10 * 1024 * 1024,
    ...
    if (
      ...
      /transaction_probe|_kysely_migration/iu.test(schemaOutput)
    ) {
      throw new Error(
        'transaction verification unexpectedly retained database objects',
      );
    }
...
if (results.some((result) => JSON.stringify(result) !== material)) {
  throw new Error('PostgreSQL transaction verification runs differ materially');
}
